...
const Header = () => {
  ...
  return (
    <div className='w-full h-16 bg-[#151515]'>
      <div className="flex justify-end items-center px-12 w-full h-full">
        <ul className='hidden md:flex'>
          {Titles.map(elem => {
            return (<li className='px-3 cursor-pointer text-white hover:text-[#8644A2]' key={elem.name}>{elem.name}</li>)
          })}
        </ul>
        ...
        <ul className={!resnav ? 'hidden' : 'absolute top-0 left-0 w-full h-screen px-3 py-2 bg-[#151515]'}>
          {Titles.map(elem => {
            return (<li className='px-3 py-1 cursor-pointer text-white hover:text-[#8644A2]' key={elem.name}>{elem.name}</li>)
          })}
        </ul>
      </div>
    </div>
